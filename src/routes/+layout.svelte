<script lang="ts">
	...
	const timeDescription = $derived(getTimeDescription(currentDateTime));

	const backgroundClass = $derived(timeDescription);

	let fontStyle = $state(
		getLocale() == 'ar' || getLocale() == 'arz' || getLocale() == 'ur' ? 'amiri-regular' : ''
	);
</script>
...
<div style="display:none">
	...
	{#each locales as locale}
		<a href={resolve(localizeHref(page.url.pathname, { locale }))}>
			<!--if your vscode also gives you an error. ignore it, i18n still works -->
			{locale}
		</a>
	{/each}
</div>
